...
To avoid loading large datasets at application startup, enable the **View Mode** display style for Lookup Property Editors that use `AllItems` data loading mode. For more information, refer to the following help topic: [Reference (Foreign Key, Complex Type) Properties](xref:113572#lookuppropertyeditors-view-mode-for-detailview-only).
