---
uid: DevExpress.Persistent.Validation.RuleIsReferencedAttribute.CriteriaEvaluationBehavior
...
> [!NOTE]
> When defining the [](xref:DevExpress.Persistent.Validation.RuleIsReferencedAttribute) in code, the value for the **CriteriaEvaluationBehavior** property is passed as a named parameter. This means that you do not have to specify it. However, when specifying the parameter, use the following format: `CriteriaEvaluationBehavior = CriteriaEvaluationBehavior.InTransaction`.