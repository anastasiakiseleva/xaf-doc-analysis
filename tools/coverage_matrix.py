...
coverage_matrix.py
==================
Builds a documentation coverage matrix from section-level concept data:
  - Section-level concept extraction (doc_concepts.parquet)
  - Document metadata (topics_inventory.parquet)
  - Aggregated ticket counts (configured via config/product.yml)
  - Support-to-concept mapping (configured via config/product.yml)

Product-specific paths and concept lists are loaded from config/product.yml
via scripts/config_loader.py — no hardcoded values in this file.
"""
...
# Paths (resolved from config/product.yml at runtime)
PROJECT_ROOT = Path(__file__).resolve().parents[1]
OUTPUT_DIR = PROJECT_ROOT / "outputs"

TAXONOMY_JSON = cfg.taxonomy_path()
SUPPORT_MAP_YML = cfg.support_concept_map_path()
TICKET_JSON = cfg.support_tickets_path()
...
def main():
    """Main execution."""
    from config_loader import _load_product
    product_name = _load_product().get("product", {}).get("name", "Product")
    print(f"Coverage Matrix Analysis — {product_name}")
    print("=" * 72)
